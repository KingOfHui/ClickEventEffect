...
public class ClickEventsCreateViewSubject implements ClickEventsViewSubject {
    ...
    public ClickEventsCreateViewSubject(ClickEventsWholeType clickEventsWholeType) {
        mClickEventsWholeType = clickEventsWholeType;
    }

    public ClickEventsCreateViewSubject(ClickEventsWholeType clickEventsWholeType, ScaleBean scaleBean) {
        mClickEventsWholeType = clickEventsWholeType;
        mScaleBean = scaleBean;
    }

    public ClickEventsCreateViewSubject(ClickEventsWholeType clickEventsWholeType, ColorBean colorBean) {
        mClickEventsWholeType = clickEventsWholeType;
    ...
    public View createView(View parent, String name, Context context, AttributeSet attrs) {
        ...
        if (type != ClickEventsManager.NONE_TYPE) {//只要在View中设置了type，除非是none，否则不论全局何种模式，都优先根据type
            ...
            if (ClickEventsManager.getViewTypes().get(name) != null) {
                view = parseTypeView(ClickEventsManager.getViewTypes().get(name),
     ...
     * @return
     */
    private View findSystemViewChange(View parent, String name, ClickEventsType wholeType, Context context, AttributeSet attrs) {
        View view = null;
        EventsAdapter adapter = null;
        if (ClickEventsManager.getListWholeType() != null
                && parent != null
                && (parent instanceof ListView
                || parent.getClass().getName().contains("widget.RecyclerView"))) {//父控件为列表，使用定义的类型
            adapter = getAdapter(ClickEventsManager.getListWholeType());
